...
#shdi
fit5<- Used ~ shdi_scaled+ (1+shdi_scaled | bird_id)
...
#pd
fit6<- Used ~ pd_scaled+ (1+pd_scaled | bird_id)
...
#ent
fit7<- Used ~ ent_scaled+ (1 +ent_scaled| bird_id)
rsf7<-glmmTMB(fit7, family = binomial, data = d,
              control = glmmTMBControl(optimizer=optim,
                                       optArgs=list(method="BFGS")))
summary(rsf7)
#ed
fit8<- Used ~ ed_scaled+ (1+ed_scaled | bird_id)
...
sink("All_RSF_models_summary_30Nov25.txt")
